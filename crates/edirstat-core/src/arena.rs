...
pub struct FileNode {
    ...
    /// Cumulative logical size in bytes.
    pub size: u64,

    /// Cumulative allocated (on-disk) size in bytes.
    pub allocated: u64,
...
impl FileNode {
    ...
    /// Size used for treemap area and default sort.
    #[must_use]
    #[inline]
    pub const fn metric(&self, use_allocated: bool) -> u64 {
        if use_allocated {
            self.allocated
        } else {
            self.size
...
mod tests {
    ...
    fn test_filenode_from_metadata() {
        ...
        assert_eq!(node.allocated, 16384);
    ...
    fn test_filenode_from_metadata_no_permission_dir_size() {
        ...
        assert_eq!(dir_node.allocated, 0);
        ...
        assert_eq!(file_node.allocated, 4096);
    }
    ...
    pub fn from_dir_entry(entry: &std::fs::DirEntry) -> Option<Self> {
        ...
        match metadata_res {
            Ok(metadata) => {
                ...
                let allocated = crate::fs_utils::allocated_len(&metadata);
