...
pub const FILE_VERSION_V4: u16 = 4;
...
/// Serializes nodes using the Version 4 format. If `compress` is enabled, the
/// output is wrapped inside a standard Zstandard container. Version 2 and 3
...
pub fn save_snapshot(
    ...
    save_snapshot_v4(nodes, string_pool, path, compress)
}

/// Serializes the snapshot in the Version 4 format to a file on disk.
pub fn save_snapshot_v4(
    nodes: &[FileNode],
    string_pool: &StringPool,
    path: &Path,
    compress: bool,
...
pub fn save_snapshot_to_bytes(
    ...
    let mut col_allocated = Vec::with_capacity(nodes.len() * 2);
    ...
    for &old_idx in &dfs_order {
        ...
        // 3. Write size and allocated (Varint)
        write_u64_varint(&mut col_size, node.size);
        write_u64_varint(&mut col_allocated, node.allocated);
    ...
    let meta_header_size = col_lengths.len() * std::mem::size_of::<u32>(); // 8 * 4 = 32 bytes
    ...
    let col_lengths_le: [u32; 8] = col_lengths.map(u32::to_le);
    ...
    edst_bytes.write_all(&col_allocated)?;
...
pub fn load_snapshot_from_bytes(
    ...
    if header.version != FILE_VERSION_V2
        && header.version != FILE_VERSION_V3
        && header.version != FILE_VERSION_V4
    {
    ...
    let decoded_nodes = if header.version == FILE_VERSION_V2 {
        ...
        // Version 3/4: Reconstruct nodes from parallel column streams.
        // V3 has 7 columns (28-byte prefix). V4 adds allocated after size (32-byte prefix).
        let is_v4 = header.version == FILE_VERSION_V4;
        let col_prefix_bytes = if is_v4 { 32 } else { 28 };
        let mut decoded =
            vec![FileNode::new(crate::arena::StringId(0), None, false, false, 0, 0); node_count];

        if decompressed_data.len() < col_prefix_bytes {
            return Err(crate::EdirstatError::TruncatedNodes);
        }
        let mut col_lengths = u8_slice_to_u32_vec(&decompressed_data[0..col_prefix_bytes]);
        ...
        let mut start = col_prefix_bytes;
        ...
        start = next;
        let col_allocated_slice = if is_v4 {
            let (slice, next) = take_column(&decompressed_data, start, col_lengths[3])?;
            start = next;
            slice
        } else {
            &[]
        ...
        for idx in 0..node_count {
            ...
            let allocated = if is_v4 {
                read_u64_varint(col_allocated_slice, &mut cursor_allocated)?
            } else {
                size
            };
...
mod tests {
    ...
    fn test_uncompressed_roundtrip() -> Result<(), crate::EdirstatError> {
        ...
        nodes[0].size = 1000;
        nodes[0].allocated = 4096;
        nodes[1].size = 1000;
        nodes[1].allocated = 4096;
        ...
        assert_eq!(loaded_nodes[0].allocated, 4096);
        assert_eq!(loaded_nodes[1].allocated, 4096);
