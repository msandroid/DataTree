...
/// Returns the on-disk allocation size for a metadata record.
///
/// Unix uses `st_blocks * 512`. Windows rounds logical size up to a 4 KiB
/// cluster. The NTFS MFT scanner supplies a more accurate allocated size for
/// whole-volume scans.
#[must_use]
pub fn allocated_len(meta: &std::fs::Metadata) -> u64 {
    #[cfg(unix)]
    {
