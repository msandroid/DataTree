...
fn propagate_all_sizes_bottom_up(arena: &mut [FileNode]) {
    for idx in (0..arena.len()).rev() {
        ...
        if parent != NO_INDEX {
            ...
            let allocated = arena[idx].allocated;
            ...
            parent_node.allocated += allocated;
