...
fn parse_attributes(record_data: &[u8]) -> SmallVec<[AttributeHeader<'_>; 6]> {
    ...
    while offset + 8 <= record_data.len() {
        ...
        let allocated_size = if is_non_resident && length >= 48 {
            u64::from_le_bytes([
                record_data[offset + 40],
                record_data[offset + 41],
                record_data[offset + 42],
                record_data[offset + 43],
                record_data[offset + 44],
                record_data[offset + 45],
                record_data[offset + 46],
                record_data[offset + 47],
            ])
        } else {
            0
        };
...
struct MetadataInfo {
    ...
    /// File size.
    size: u64,
    /// Allocated on-disk size (clusters), including named data streams.
    allocated: u64,
...
fn extract_metadata_info(
    ...
    let mut unnamed_allocated = 0u64;
    let mut named_allocated = 0u64;
    ...
    for attr in attrs {
        match attr.ty {
            ...
            0x80 if attr.name_length == 0 => {
                ...
                unnamed_allocated = unnamed_allocated.max(attr.allocated_size);
            }
            0x80 => {
                ...
                scope.spawn(move |_| {
                    ...
                    for (i, entry_slot) in target_slice.iter_mut().enumerate() {
                        ...
                        *entry_slot = Some(MftEntry {
                            size,
                            allocated: if is_dir { 0 } else { metadata.allocated },
                            parent_record_id,
        ...
        *entry_slot = None;
        ...
        match (untrusted_size, untrusted_parent_size) {
            ...
            (false, true) => {
                ...
                parent.allocated = extension.allocated;
    ...
    while let Some(frame) = stack.pop() {
        ...
        while child_record != crate::arena::NO_INDEX {
            ...
            if is_dir {
                ...
                let mut actual_allocated = entry.allocated;
                    ...
                    && let Some(name_str) = sharded_pool.get_compact_str(entry.name_id)
                {
                    ...
                    if let Ok(meta) = std::fs::metadata(&full_path) {
                        ...
                        actual_allocated = crate::fs_utils::allocated_len(&meta);
                    }
                }
