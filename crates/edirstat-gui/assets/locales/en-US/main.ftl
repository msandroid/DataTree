...
modal-elevation-desc = DataTree runs with standard user privileges by default. However, Windows strictly restricts raw physical disk handle access to administrator accounts.
...
# About Modal
modal-about-title = ℹ About DataTree
modal-about-author = By: Cody Wyatt Neiman (xangelix) <neiman@cody.to>
modal-about-license-btn = 📜 License (MIT)
modal-about-desc1 = DataTree is a high-performance disk space analyzer built on the eDirStat engine, with WizTree-style File View, Allocated size, and CSV export.
...
modal-licenses-tab-app = DataTree / eDirStat (MIT)
modal-licenses-tab-deps = Third-Party Libraries
modal-licenses-app-desc = DataTree is based on eDirStat and is open source software distributed under the MIT License:
...
explorer-hdr-allocated = Allocated
...
explorer-tab-tree = Tree View
explorer-tab-files = File View
file-view-empty = No files to display
export-csv = Export CSV
treemap-metric-allocated = Treemap uses Allocated
scan-engine-mft = MFT
scan-engine-walk = Walk
scan-engine-label = Engine: { $engine }
explorer-grid-allocated = Allocated:
explorer-total-allocated = Total Allocated: { $size }
