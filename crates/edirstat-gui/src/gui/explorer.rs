...
impl egui_table_kit::operations::TableProvider for TableProviderWrapper<'_> {
    ...
    fn header(&self, index: usize) -> Option<std::borrow::Cow<'_, str>> {
        let key = match index {
            ...
            2 => "explorer-hdr-size",
            3 => "explorer-hdr-allocated",
            4 => "explorer-hdr-items",
            5 => "explorer-hdr-files",
            6 => "explorer-hdr-subdirs",
            7 => "explorer-hdr-created",
            8 => "explorer-hdr-modified",
            _ => return None,
        };
        Some(t!(key))
    }
    ...
    fn cell_at(
        ...
        let val = match col_index {
            ...
            1 => {
                let pct = percent_of_parent(self.snapshot, node);
            ...
            3 => std::borrow::Cow::Owned(
                prettier_bytes::ByteFormatter::new()
                    .format(node.allocated)
                    .to_string(),
            ),
            4 => {
    ...
    fn row_children(&self, row_index: usize) -> Vec<usize> {
        ...
        // Sort initial list by allocated descending (WizTree-style default)
        children.sort_by(|&a, &b| {
            self.snapshot.nodes[b]
                .allocated
                .cmp(&self.snapshot.nodes[a].allocated)
                .then_with(|| {
                    self.snapshot.nodes[b]
                        .size
    ...
    fn row_matches(
        ...
        for &(col_idx, ref filter) in filters {
            let cell_text = match col_idx {
                ...
                2 => prettier_bytes::ByteFormatter::new()
                    .format(node.size)
                    .to_string(),
                3 => prettier_bytes::ByteFormatter::new()
                    .format(node.allocated)
                    .to_string(),
                4 => {
                    if node.is_directory() {
...
impl QueryCoordinator {
    ...
    pub fn update(
        ...
        if !search_query.is_empty() && !snapshot.nodes.is_empty() {
            let parsed = if filter_regex {
                None
            } else {
                Some(crate::gui::search::SearchQuery::parse(search_query))
            };
            ...
            for idx in (0..snapshot.nodes.len()).rev() {
                ...
                let self_matches = if let Some(parsed) = parsed.as_ref() {
                    parsed.matches_name(&cleaned_name, filter_case_sensitive)
                        && parsed.matches_sizes(node.size, node.allocated)
                } else {
                    regex_matcher.as_ref().map_or_else(
                        || {
...
impl GuiApp {
    ...
    fn flatten_visible_tree_impl(
        ...
        if is_expanded && has_children {
            ...
            if let Some((sort_col, sort_up)) = sort_state {
                ...
                // Default: Sort immediate children by allocated descending
                sorted_child_indices.sort_by(|&a, &b| {
    ...
    pub fn render_hierarchical_table(&mut self, ui: &mut egui::Ui, snapshot: &FileArenaSnapshot) {
        ...
        if self.table_state.columns.len() < 9 {
            self.table_state
                .columns
                .resize_with(9, egui_table_kit::header::ColumnState::default);
        }
        ...
        let custom_cell_ui = Box::new(
            move |ui: &mut egui::Ui,
                ...
                if cell_info.col_nr == 1 {
                    ...
                    let parent_metric = if node.parent == crate::arena::NO_INDEX {
                        node.allocated.max(node.size).max(1)
                    } else {
                        let parent = &snapshot_nodes[node.parent as usize];
    ...
    pub fn render_multi_file_detail_list(
        ...
        for &root_idx in &roots {
            if (root_idx as usize) < snapshot.nodes.len() {
                ...
                total_allocated += snapshot.nodes[root_idx as usize].allocated;
            }
        }
        ...
        let total_size_str = prettier_bytes::ByteFormatter::new()
            .format(total_size)
            .to_string();
        let total_allocated_str = prettier_bytes::ByteFormatter::new()
        ...
        ui.vertical(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                ui.label(t!("explorer-total-allocated", {
                    "size" => total_allocated_str.as_str()
                }));
                ...
                ui.vertical(|ui| {
                    ...
                    egui::Grid::new("file_details_grid")
                        ...
                        .show(ui, |ui| {
                            ...
                            // Allocated field
                            ui.weak(t!("explorer-grid-allocated"));
                            let formatted_allocated = prettier_bytes::ByteFormatter::new()
                                .format(node.allocated)
                                .to_string();
                            ui.allocate_ui(
                                egui::vec2(ui.available_width(), ui.spacing().interact_size.y),
...
pub fn compare_nodes_by_column(
    ...
    match col_index {
        ...
        3 => {
            // Sort by Allocated
            node_a.allocated.cmp(&node_b.allocated)
        }
        4 => {
        ...
        _ => {
            ...
            node_a
                .allocated
                .cmp(&node_b.allocated)
                .then_with(|| node_a.size.cmp(&node_b.size))
        }
    }
}

/// Percentage of parent allocated size (falls back to logical size when allocated is 0).
#[must_use]
fn percent_of_parent(snapshot: &FileArenaSnapshot, node: &crate::arena::FileNode) -> f32 {
    ...
    let child = if node.allocated == 0 {
        node.size
    } else {
        node.allocated
    };
    #[allow(clippy::cast_precision_loss)]
    (child as f32 / parent_metric as f32).clamp(0.0, 1.0)
}
...
mod tests {
    ...
    fn test_compare_nodes_by_column() {
        ...
        // Column 3: allocated.
        assert_eq!(compare_nodes_by_column(&snapshot, 3, 1, 2), Ordering::Less);
        assert_eq!(
            compare_nodes_by_column(&snapshot, 3, 3, 2),
            Ordering::Greater
        );

        // Column 8: modified timestamp.
        assert_eq!(
            compare_nodes_by_column(&snapshot, 8, 1, 2),
            Ordering::Greater
        );
        assert_eq!(compare_nodes_by_column(&snapshot, 8, 2, 3), Ordering::Less);

        // Column 5: file count; non-directories count as 0.
        assert_eq!(
            compare_nodes_by_column(&snapshot, 5, 1, 2),
            Ordering::Greater
        );
        assert_eq!(compare_nodes_by_column(&snapshot, 5, 3, 1), Ordering::Less);

        // Out-of-range column index falls back to allocated, then size.
