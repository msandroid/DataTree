...
pub struct TreemapChart {
    ...
    pub use_allocated: bool,
    cached_use_allocated: bool,
}
...
impl TreemapChart {
    ...
    pub const fn new() -> Self {
        Self {
            ...
            use_allocated: false,
            cached_use_allocated: false,
        }
    }

    /// Layout the treemap into a fixed rectangle for `--bench-ui` (no GPU).
    pub fn bench_layout(
        &mut self,
        ...
        self.last_rect = Rect::from_min_size(pos2(0.0, 0.0), eframe::egui::vec2(width, height));
        self.use_allocated = true;
        let blocks = StatsChart::compute(self, snapshot);
        let n = blocks.len();
...
impl StatsChart for TreemapChart {
    ...
    fn compute(&mut self, snapshot: &crate::arena::FileArenaSnapshot) -> Self::Output {
        ...
        if (self.zoom_root as usize) >= snapshot.nodes.len()
            || snapshot.nodes[self.zoom_root as usize].metric(self.use_allocated) == 0
        ...
        let config = TreemapConfig {
            ...
            max_depth: 20,
            use_allocated: self.use_allocated,
        };
...
impl StatComponent for TreemapChart {
    fn render(
        ...
        let needs_rebuild = self.cached_blocks.is_empty()
            ...
            || self.use_allocated != self.cached_use_allocated;
        ...
        if needs_rebuild {
            ...
            self.cached_use_allocated = self.use_allocated;
        }
        ...
        if let Some(block) = hovered_block {
            ...
            let size_str = prettier_bytes::ByteFormatter::new()
                .format(snapshot.nodes[block.node_idx as usize].metric(self.use_allocated))
                .to_string();
...
fn build_treemap(
    ...
    if node.metric(config.use_allocated) == 0
    ...
    children.sort_by(|&a, &b| {
        config.nodes[b as usize]
            .metric(config.use_allocated)
            .cmp(&config.nodes[a as usize].metric(config.use_allocated))
    });

    let active_children: Vec<u32> = children
        .into_iter()
        .filter(|&idx| config.nodes[idx as usize].metric(config.use_allocated) > 0)
        .collect();
    ...
    let total_size = active_children
        .iter()
        .map(|&idx| config.nodes[idx as usize].metric(config.use_allocated))
        .sum::<u64>() as f64;
    ...
    let child_areas: Vec<f64> = active_children
        ...
        .map(|&idx| {
            #[allow(clippy::cast_precision_loss)]
            let size = config.nodes[idx as usize].metric(config.use_allocated) as f64;
